...
using Plots, Images, ArchGDAL, Flux, Augmentor, MLUtils
...
begin 
	...
	FourPatch() = FourPatch([(row, col) for col in 1:512:513 for row in 1:512:513])
...
begin 
	...
	NinePatch() = NinePatch([(row, col) for col in 1:256:513 for row in 1:256:513])

	function (p::NinePatch)(img::Array{Float32, 3}, patch::Int)
		@assert 1 <= patch <= 9
		row, col = p.coords[patch]
		return img[row:row+511,col:col+511,:]
	end
...
function show_patching_strategies(tile::Int)	
	...
	nir_plot_tile = @pipe read_nir(tile) |> plot_gray |> put_title!(_, "NIR")
	swir_plot_tile = @pipe read_swir(tile) |> plot_gray |> put_title!(_, "SWIR")
	mask_plot_tile = @pipe read_mask(tile) |> plot_gray |> put_title!(_, "Mask")
	
	# Plot Four Patch Strategy
	rgb_plot_4 = @pipe read_rgb(tile) |> plot_four_patch(_, plot_color)
	nir_plot_4 = @pipe read_nir(tile) |> plot_four_patch(_, plot_gray)
	swir_plot_4 = @pipe read_swir(tile) |> plot_four_patch(_, plot_gray)
	mask_plot_4 = @pipe read_mask(tile) |> plot_four_patch(_, plot_gray)

	# Plot Nine Patch Strategy
	rgb_plot_9 = @pipe read_rgb(tile) |> plot_nine_patch(_, plot_color)
	nir_plot_9 = @pipe read_nir(tile) |> plot_nine_patch(_, plot_gray)
	swir_plot_9 = @pipe read_swir(tile) |> plot_nine_patch(_, plot_gray)
	mask_plot_9 = @pipe read_mask(tile) |> plot_nine_patch(_, plot_gray)
	
	# Plot All Tiles
	plots = [
		rgb_plot_tile, nir_plot_tile, swir_plot_tile, mask_plot_tile, 
		rgb_plot_4, nir_plot_4, swir_plot_4, mask_plot_4, 
		rgb_plot_9, nir_plot_9, swir_plot_9, mask_plot_9, 
	]
...
# ╔═╡ 74803bb2-5c0e-4225-9707-5fa3909e41e8
function predict(model, x)
	model(x) |> prediction_to_mask
end;

# ╔═╡ 84ad4489-7d5e-4166-9a58-b530f7014ef6
...
function true_positive(ŷ::Vector{<:Number}, y::Vector{<:Number})
	ŷ .* y |> sum
end;

# ╔═╡ c26f6963-05a7-4fa6-a6d6-978b48784c08
function false_positive(ŷ::Vector{<:Number}, y::Vector{<:Number})
	ŷ .* (1 .- y) |> sum
end;

# ╔═╡ ae774bdc-a62b-4bd5-a238-69b3c6e190f0
function false_negative(ŷ::Vector{<:Number}, y::Vector{<:Number})
	(1 .- ŷ) .* y |> sum
end;

# ╔═╡ 023ae564-36b3-4b2d-9ede-c2feca60c672
function recall(ŷ::Vector{<:Number}, y::Vector{<:Number})
	TP = true_positive(ŷ, y)
	FN = false_negative(ŷ, y)
	return (TP + eps(typeof(TP))) / (TP + FN + eps(typeof(TP)))
end;

# ╔═╡ d05be475-5bf4-4604-8746-f44bd80f861e
function precision(ŷ::Vector{<:Number}, y::Vector{<:Number})
	TP = true_positive(ŷ, y)
	FP = false_positive(ŷ, y)
	return (TP + eps(typeof(TP))) / (TP + FP + eps(typeof(TP)))
end;

# ╔═╡ be6b5cfc-4eb5-43f7-8b32-4af2ab41c526
function IoU(ŷ::Vector{<:Number}, y::Vector{<:Number})
	TP = true_positive(ŷ, y)
	FP = false_positive(ŷ, y)
	FN = false_negative(ŷ, y)
	ϵ = eps(typeof(TP))
...
begin
	...
	function (m::SegmentationMetric)(ŷ::Array{<:Number,4}, y::Array{<:Number,4})
		@assert size(ŷ) == size(y)
		@assert length(m.weights) == size(ŷ)[3]
		rows, cols, nclasses, obs = size(ŷ)
		ŷ = hcat([ŷ[r,c,:,o] for r in 1:rows for c in 1:cols for o in 1:obs]...)
		y = hcat([y[r,c,:,o] for r in 1:rows for c in 1:cols for o in 1:obs]...)
		metrics = [m.metric(ŷ[class,:], y[class,:]) for class in 1:nclasses]
		return sum(metrics .* m.weights)
	end
...
begin
	...
	# Get Parameters
	params = Flux.params(model[1:end-1])
end;
...
begin
	...
	true_positive(y_pred, y_true) |> println   # 2.0
	false_positive(y_pred, y_true) |> println  # 1.0
	false_negative(y_pred, y_true) |> println  # 1.0
	recall(y_pred, y_true) |> println          # 0.66
	precision(y_pred, y_true) |> println       # 0.66
	IoU(y_pred, y_true) |> println             # 0.5

	y_true_mask = zeros(Float32, (2, 2, 2, 1))
	y_true_mask[1,1,2,1] = 1.0
	y_true_mask[1,2,2,1] = 1.0
	y_true_mask[2,1,2,1] = 1.0
	y_true_mask[2,2,1,1] = 1.0
	
	y_pred_mask = zeros(Float32, (2, 2, 2, 1))
	y_pred_mask[1,1,2,1] = 1.0
	y_pred_mask[1,2,1,1] = 1.0
	y_pred_mask[2,1,2,1] = 1.0
	y_pred_mask[2,2,2,1] = 1.0
	
	recall_mask = SegmentationMetric(recall, [0.0, 1.0])
	recall_mask(y_true_mask, y_pred_mask) |> println

	precision_mask = SegmentationMetric(precision, [0.0, 1.0])
	precision_mask(y_true_mask, y_pred_mask) |> println

	IoU_mask = SegmentationMetric(IoU, [0.5, 0.5])
	IoU_mask(y_true_mask, y_pred_mask) |> println

	function onehot_mask(mask, nclasses::Int)
		rows, cols, _, obs = size(mask)
		mask_out = zeros(Float32, (rows, cols, nclasses, obs))
		for (r, c, o) in [(r, c, o) for r in 1:rows for c in 1:cols for o in 1:obs]
			onehot_index = mask[r,c,1,o] + 1 |> floor |> Int
	...
	y_true = @pipe [1.0 1.0; 1.0 0.0] |> reshape(_, (2, 2, 1, 1))
	y_pred = [1.0 0.0; 1.0 1.0]
	onehot_mask(y_true, 2)
...
md"""
# Visualize Patching Strategies
"""

# ╔═╡ 1b5a025f-4900-4977-8f7d-72bd5679e932
md"""
# Define Data Pipeline
I found that parallelizing the for-loop reduced the time required to read a batch of 32 samples from about 1 second to 0.6 seconds, a saving of 0.4 seconds per-batch. When we consider that training typically involves iterating over thousands of batches, we realize that the savings due to parallelization are significant.
"""
...
git-tree-sha1 = "58d7d54715f6bb37b4dde1dab0781c75dbb6d02d"
...
git-tree-sha1 = "81549677be5d9023f01bcc71dace20d267e817db"
...
[[deps.UNet]]
deps = ["Distributions", "FileIO", "Flux", "ImageCore", "ImageTransformations", "Reexport", "Statistics", "StatsBase"]
git-tree-sha1 = "480b8fd23b1f4c7b9395f2f70811ad6581b2cdc2"
